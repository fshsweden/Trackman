...
namespace Trackman.Tracker
{
	...
	internal sealed class Tracker : StatefulService, ILocationReporter, ILocationViewer
	{
		 ...
		 * 
		 * <summary>
		 * Reports a Targets location
		 * Location consists of TargetID, Latitude and Longitude
		 * </summary>
		 * 
		 */
		public async Task ReportLocation(Location location)
		{
			...
			using (var tx = StateManager.CreateTransaction())
			{
				...
				// Get the list of Actors (indexed by Guid)
				var TargetIds = await StateManager.GetOrAddAsync<IReliableDictionary<Guid, ActorId>>("TargetIds");

				var timestamp = DateTime.UtcNow;

				// Update Target (add TargetId with a random Actor ID if we dont already have it)
				var TargetActorId = await TargetIds.GetOrAddAsync(
					tx,
					location.TargetId,
					ActorId.CreateRandom()	// if it doesnt exists, add this value for the key!
				);

				// Create an ActorProxy and update it with timestamp, lat and long
